// IRLMod | player-client.js (v2.59.5 - WSS & Robust Perf Mode)
...
// Value: {
//   canvasCoords: {x, y}, 
//   startCanvasCoords: {x,y}, 
//   startTime: timestamp,     
//   selectedToken: foundryTokenObject || null, 
//   isDraggingConfirmed: false, 
//   dragOffset: {x:0, y:0}    
// }
let lastPiFocusedToken = null; // Token whose vision/FoW is shown, set by a TAP
...
function connectToPiServer() {
    ...
    // Determine protocol based on current page's protocol
    const pageProtocol = window.location.protocol;
    const wsProtocol = pageProtocol === "https:" ? "wss://" : "ws://";
    
    const wsUrl = `${wsProtocol}${piServerIP}:${piServerPort}`;
...
async function applyPlayerPerformanceMode() {
    ...
    try {
        const desiredModeKey = game.settings.get(MODULE_ID, "playerPerformanceMode"); 
        if (!desiredModeKey) { 
            console.warn("IRLMod | Player Client: Player performance mode module setting not retrieved.");
            return; 
        }
        // console.log(`IRLMod | Player Client: Desired performance mode key from settings: '${desiredModeKey}'`);

        let targetPerformanceNumericalValue;
        const desiredModeLower = desiredModeKey.toLowerCase();

        switch (desiredModeLower) {
            case "low": targetPerformanceNumericalValue = CONST.CANVAS_PERFORMANCE_MODES.LOW; break;
            case "medium": targetPerformanceNumericalValue = CONST.CANVAS_PERFORMANCE_MODES.MEDIUM; break;
            case "high": targetPerformanceNumericalValue = CONST.CANVAS_PERFORMANCE_MODES.HIGH; break;
            case "maximum": 
                targetPerformanceNumericalValue = CONST.CANVAS_PERFORMANCE_MODES.MAXIMUM !== undefined 
                    ? CONST.CANVAS_PERFORMANCE_MODES.MAXIMUM 
                    : CONST.CANVAS_PERFORMANCE_MODES.HIGH; 
                if (CONST.CANVAS_PERFORMANCE_MODES.MAXIMUM === undefined && desiredModeLower === "maximum") {
                    // console.warn("IRLMod | Player Client: 'Maximum' performance mode const not defined, falling back to 'High'.");
                }
                break;
            case "auto":
            default:
                targetPerformanceNumericalValue = CONST.CANVAS_PERFORMANCE_MODES.AUTO;
                if (desiredModeLower !== "auto") {
                     console.warn(`IRLMod | Player Client: Invalid performance mode string '${desiredModeLower}'. Defaulting to 'Auto' (${targetPerformanceNumericalValue}).`);
                }
                break;
        }
        
        const currentCoreModeNumerical = game.settings.get("core", "performanceMode"); 

        if (currentCoreModeNumerical !== targetPerformanceNumericalValue) {
            // console.log(`IRLMod | Player Client: Current core performance mode: '${currentCoreModeNumerical}'. Attempting to set to numerical value: '${targetPerformanceNumericalValue}' (for desired key '${desiredModeKey}').`);
            try {
                // Use the numerical value for setting, as this seems more robust internally for "core.performanceMode"
                await game.settings.set("core", "performanceMode", targetPerformanceNumericalValue);
                
                const newCoreMode = game.settings.get("core", "performanceMode");
                if (newCoreMode === targetPerformanceNumericalValue) {
                    // console.log(`IRLMod | Player Client: Core performance mode successfully set to '${newCoreMode}'.`);
                } else {
                    console.warn(`IRLMod | Player Client: Core performance mode after set is '${newCoreMode}' (requested numerical '${targetPerformanceNumericalValue}').`);
                }
                ...
                if (canvas?.performance && canvas.performance.mode !== targetPerformanceNumericalValue) {
                    ...
                    // console.log(`IRLMod | Player Client: Directly set canvas.performance.mode to ${canvas.performance.mode}.`);
                }
                ...
                if (canvas?.ready) { 
                    ...
                    } catch (refreshErr) { console.error("IRLMod | Player Client: Error during canvas refresh after performance mode change:", refreshErr); }
                }
            } catch (setErr) { console.error(`IRLMod | Player Client: Error during game.settings.set for performanceMode:`, setErr); }
        }
    } catch (err) { console.error("IRLMod | Player Client: Outer error applying performance mode:", err); }
}
...
Hooks.once('init', () => { 
    ...
    game.settings.register(MODULE_ID, "playerPerformanceMode", { name: "irlmod.settingPlayerPerformanceMode.name", hint: "irlmod.settingPlayerPerformanceMode.hint", scope: "world", config: true, type: String, default: "high", 
        choices: {"low": "IRLMOD.performanceModes.low", "medium": "IRLMOD.performanceModes.medium", "high": "IRLMOD.performanceModes.high", "maximum": "IRLMOD.performanceModes.maximum", "auto": "IRLMOD.performanceModes.auto"} 
    });
...
function reapplyPlayerViewRestrictions() { 
    if (game.user && game.user.name === DEDICATED_PLAYER_USER_NAME) {
        ...
        startPersistentForceHide(); // Ensure this function is defined before this hook
        if (canvas?.ready) { 
            disablePlayerCanvasInteractions(); // Ensure this function is defined before this hook
        }
    }
}
...
console.log("IRLMod | Player Client Script Loaded (v2.59.5 - Function Order & Robust Perf Mode)");