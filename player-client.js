...
function connectToPiServer() {
    ...
    console.log(`IRLMod | Player Client: Connecting to Pi Server via ${wsUrl}`);

    try {
        piWebSocket = new WebSocket(wsUrl);
    } catch (e) {
        console.error(`IRLMod | Player Client: Error creating WebSocket: ${e}.`); return;
    }
    ...
    piWebSocket.onerror = (error) => {
        console.error("IRLMod | Player Client: WebSocket error with Pi server:", error);
        if (isSecure) {
            console.warn("IRLMod | Player Client: You are on HTTPS. Ensure the Pi server is running with --ssl and you have accepted the certificate (e.g. by visiting https://<PI_IP>:<PORT> in a separate tab).");
        }
    };
...
function handlePiTouchMove(touch_id, canvasCoords, time) {
    ...
    if (touchData.isDraggingConfirmed) {
        ...
        try {
            ...
            if (!touchData.positionCommitTimer) {
                touchData.positionCommitTimer = setTimeout(() => {
                    touchData.positionCommitTimer = null;
                    const tok = touchData.selectedToken;
                    if (!tok || !canvas.tokens.get(tok.id)) return;
                    tok.document.update(
                        { x: tok.document.x, y: tok.document.y },
                        { animate: false }
