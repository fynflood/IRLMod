// IRLMod | gm-screen.js (v35 - Settings & Launch Fixes)
...
// TV Settings - These will be populated from settings
let tvPhysicalWidthInches;
let tvResolutionWidthPixels;
let tvDesiredGridInches;

// Pi Server settings - These will be populated from settings
let piServerIP;
let piServerPort;         
...
function injectGMOverlayCSS() {
    ...
    const css = `
        #irlmod-gm-overlay { position: fixed; pointer-events: none; z-index: 9998; box-sizing: border-box; background-size: contain; background-repeat: no-repeat; background-position: center; }
        #irlmod-gm-overlay-header { position: absolute; top: 0; left: 0; width: 100%; height: ${headerBarHeight}; background-color: rgba(40, 40, 40, 0.95); color: #e0e0e0; padding-left: 10px; font-size: 13px; font-weight: bold; line-height: ${headerBarHeight}; border-radius: 4px 4px 0 0; cursor: move; pointer-events: auto; white-space: nowrap; user-select: none; box-sizing: border-box; overflow: hidden; text-overflow: ellipsis; display: flex; align-items: center; border-bottom: 1px solid rgba(20,20,20,0.7); }
        ...
        #irlmod-gm-overlay-content-area { position: absolute; top: ${headerBarHeight}; left: 0; width: 100%; height: calc(100% - ${headerBarHeight}); border: ${overlayBorderWidth} dashed dodgerblue; background-color: rgba(0, 100, 255, 0.05); box-sizing: border-box; pointer-events: none; }
        #irlmod-gm-overlay.splash-active #irlmod-gm-overlay-content-area { border-style: solid; border-color: limegreen; background-color: rgba(0, 255, 0, 0.03); }
        #irlmod-gm-overlay-controls { position: absolute; bottom: -33px; left: 50%; transform: translateX(-50%); background-color: rgba(40, 40, 40, 0.95); padding: 5px 8px; border-radius: 0 0 5px 5px; pointer-events: auto; display: flex; gap: 8px; border-top: 1px solid rgba(20,20,20,0.7); }
        ...
        .irlmod-resize-handle { position: absolute; width: 12px; height: 12px; background-color: rgba(0,100,200,0.6); border: 1px solid rgba(255,255,255,0.7); pointer-events: auto; z-index: 9999; box-sizing: border-box; }
...
function renderGMOverlayFromCanvasState() { 
    ...
    gmOverlayDiv.style.left = `${(gmOverlayCanvasState.x * wt.a) + wt.tx}px`;
    gmOverlayDiv.style.top = `${(gmOverlayCanvasState.y * wt.d) + wt.ty}px`;
    ...
    if (gmOverlayContent) { 
        gmOverlayDiv.style.backgroundImage = (isSplashActiveOnPlayer && splashImageURL) ? `url("${splashImageURL}")` : 'none';
        gmOverlayDiv.classList.toggle('splash-active', isSplashActiveOnPlayer && splashImageURL);
    }
}

function calibrateOverlayForTVGrid() { 
    if (!canvas || !canvas.ready || !canvas.dimensions || !canvas.scene?.grid?.size) { 
        ui.notifications.error("IRLMod: Canvas or scene grid not ready for calibration."); return; 
    }
    ...
    if (!tvPhysicalWidthInches || !tvResolutionWidthPixels || !tvDesiredGridInches || 
        tvPhysicalWidthInches <= 0 || tvResolutionWidthPixels <= 0 || tvDesiredGridInches <= 0) {
        ui.notifications.error("IRLMod: TV dimension or desired grid size settings are missing, invalid, or not positive. Please check module settings."); 
        return;
    }
...
function toggleGMOverlay() { 
    ...
    if (!gmOverlayDiv) {
        ...
        gmOverlayHeader.textContent = game.i18n.localize("IRLMOD.OverlayHeaderTitle"); 
    ...
    if (gmOverlayCanvasState.visible && (!gmOverlayCanvasState.width || gmOverlayCanvasState.width < 10)) { // Re-initialize if seems broken
        if (canvas && canvas.ready && canvas.dimensions) {
            const viewRect = canvas.dimensions.rect; 
            gmOverlayCanvasState.width = viewRect.width * 0.6; 
            gmOverlayCanvasState.height = gmOverlayCanvasState.width / (16/9);
...
function makeDraggable(element, handle) { /* Same as v34 */
    ...
    function onMouseMove(e) {
        if (!isDragging || !canvas || !canvas.ready || !canvas.stage?.scale?.x || canvas.stage.scale.x === 0) return; // Added more checks for scale
...
function makeResizable(element, handle) { /* Same as v34 */
    ...
    function doResize(e) {
        if(!isResizing || !canvas || !canvas.ready || !canvas.stage?.scale?.x || canvas.stage.scale.x === 0) return; // Added more checks for scale
        let nCW = initialCanvasWidth + ((e.clientX - startMouseScreenX) / canvas.stage.scale.x);
        const minWidthCanvas = 160 / canvas.stage.scale.x; // Ensure min width in canvas units
...
function sendOverlayViewToPlayerScreen() { /* Same as v34 */
    console.log("IRLMod | GM: sendOverlayViewToPlayerScreen called.");
...
function togglePlayerSplashImage() { /* Same as v34 */
...
function launchOrFocusPlayerView() {
    ...
    if (playerWindowRef && !playerWindowRef.closed) {
        ...
        ui.notifications.info(`IRLMod: Player View window focused. Target user: '${dedicatedPlayerUsername}'.`);
    } else {
        playerWindowRef = window.open(joinUrl, PLAYER_WINDOW_NAME, 'width=1280,height=720,noopener,noreferrer');
        if (playerWindowRef) { // window.open can return null if blocked
            ui.notifications.info(`IRLMod: Player View window opened (or attempt initiated). Target user: '${dedicatedPlayerUsername}'.`);
        } else {
            ui.notifications.error(`IRLMod: Failed to open Player View window. Pop-ups might be blocked by your browser.`);
        }
    }
}
...
Hooks.once('init', () => {
    ...
    game.settings.register(MODULE_ID, "splashImageURL", {
        name: "irlmod.settingSplashImageURL.name", hint: "irlmod.settingSplashImageURL.hint", 
    ...
    game.settings.register(MODULE_ID, "piServerIP", {
        name: "irlmod.settingPiServerIP.name", hint: "irlmod.settingPiServerIP.hint", 
    ...
    game.settings.register(MODULE_ID, "piServerPort", {
        name: "irlmod.settingPiServerPort.name", hint: "irlmod.settingPiServerPort.hint", 
    ...
    game.settings.register(MODULE_ID, "playerPerformanceMode", {
        name: "irlmod.settingPlayerPerformanceMode.name", hint: "irlmod.settingPlayerPerformanceMode.hint",
        scope: "world", config: true, type: String, default: "high",
        choices: { // Use localization keys directly here
            "low": "IRLMOD.performanceModes.low",
            "medium": "IRLMOD.performanceModes.medium",
            "high": "IRLMOD.performanceModes.high",
            "auto": "IRLMOD.performanceModes.auto"
        }
    });

    // Load initial values AFTER registration
    ...
    console.log(`IRLMod | GM Init hook finished (v35). Settings registered & loaded. Pi Server: ${piServerIP}:${piServerPort}`);
});
...
Hooks.on("canvasReady", () => {
    if (gmOverlayDiv && gmOverlayCanvasState.visible) {
        const defaultX = 500, defaultY = 500; 
        if (gmOverlayDiv.style.display !== 'none' && 
            Math.abs(gmOverlayCanvasState.x - defaultX) < 1 && 
            Math.abs(gmOverlayCanvasState.y - defaultY) < 1 &&
            (!gmOverlayCanvasState.width || gmOverlayCanvasState.width < 10 )) { // Check if width is uninitialized too
             if (canvas && canvas.ready && canvas.dimensions) {
                const viewRect = canvas.dimensions.rect;
                gmOverlayCanvasState.width = viewRect.width * 0.6; 
                gmOverlayCanvasState.height = gmOverlayCanvasState.width / (16/9);
...
Hooks.on("renderSceneControls", (sceneControlsApp, htmlElement, data) => { /* Same as v34 */
...
console.log("IRLMod | GM Screen Script Loaded (v35 - Settings & Launch Fixes)");